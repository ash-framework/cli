...
function copy (src, name, dest) {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(src, 'utf8', function (err, data) {
      ...
      const result = data.replace('<%= name %>', name)
...
function validateType (type) {
  if (['route', 'middleware', 'mixin', 'initializer', 'service', 'model'].indexOf(type) < 0) {
    return Promise.reject(new Error('Expected `type` to be one of `route`, `middleware`, `mixin`, `initializer`, `service` or `model`'))
  }
  return type
}
...
Promise.resolve()
  ...
  .then(() => {
    ...
    if (type === 'route') {
      ...
      file = path.join(blueprintsDir, 'model', 'model.js')
      destinations.push(`${name}.js`)
    }
    return Promise.all(destinations.map(dest => {
      messages.push(`Generated ${dest}`)
      return copy(file, classify(name), path.join(directoriesByType[type], dest))
    }))
    .then(() => {
      return messages
    })
